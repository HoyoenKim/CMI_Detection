    ...
    for feature_extractor in "${feature_extractors[@]}"; do
        for model in "${models[@]}"; do
            for decoder in "${decoders[@]}"; do
                ...
                echo "python3 main.py \"train\" 0 1 $dataset $feature_extractor $model $decoder $env $num_workers $downsample_rate $batch_size $distance $score_th"
                python3 main.py "train" 0 1 $dataset $feature_extractor $model $decoder $env $num_workers $downsample_rate $batch_size $distance $score_th
            done
        done
    done
done